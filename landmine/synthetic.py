...
def _company(profile: str, i: int, rng: random.Random) -> list[Fact]:
    ...
    def add_inst(concept, val):      # instant fact at the latest period
    ...
    add_inst(TOTAL_ASSETS, assets)
    add_inst(CURRENT_ASSETS, cur_a)
    add_inst(CURRENT_LIABILITIES, cur_l)
    add_inst(CASH, cash)
    add_inst(STOCKHOLDERS_EQUITY, equity)
