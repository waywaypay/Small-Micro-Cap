...
`landmine_mcp/` is a local [Model Context Protocol](https://modelcontextprotocol.io)
server that wraps the deployed API so an MCP host (Claude Desktop, etc.) can run
the screen as a tool. It's a thin client — each tool POSTs to the FastAPI service
and returns its scorecard JSON unchanged.
