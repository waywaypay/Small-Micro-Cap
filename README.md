...
- `LANDMINE_MAX_UNIVERSE` — cap on names a single `/universe` request will
  fetch + score (default 3000), so a wide cap band can't run away. Raised from
  250 so the full $50M-$2B micro/small-cap band (~2,649 names) is allowed; note
  a synchronous `/universe` screen of thousands of names is one long request, so
  for very wide sweeps drive `/run` in batches (or an async orchestrator) rather
  than a single call.
- `LANDMINE_MAX_WORKERS` — fan-out width for the per-name SEC fetches in both
  the size-cut and the screen (default 8). Each name is an independent network
  fetch, so a bounded pool overlaps the round-trips instead of running them
  strictly back-to-back; output stays byte-identical (results are ordered by
  sorted ticker before serialising).
- `LANDMINE_SEC_RPS` — shared ceiling on SEC requests/second across all workers
  (default 8, under SEC fair-access's ~10/s). A process-global limiter paces
  only real network calls — cache hits are never throttled — so parallelism
  speeds a sweep up without risking an IP throttle.
- `LANDMINE_CACHE_DIR` — where `HttpCompanyFactsProvider` caches companyfacts
  JSON (default `out/companyfacts_cache`). Point it at a persistent disk so a
  re-run after any interruption resumes from cache instead of re-fetching.

### Deploy to Render

`render.yaml` is a Blueprint: `pip install -e .` to build, `uvicorn
landmine_api.app:app --host 0.0.0.0 --port $PORT` to run, `/health` as the
health check. `API_KEY` is generated by Render; `SEC_USER_AGENT` is left unset
(`sync: false`) for you to fill in. Push the repo, then in the Render dashboard
choose **New + → Blueprint** and select it.

It targets the paid **Starter** plan with a mounted **persistent disk** (at
`/var/cache/landmine`, wired to `LANDMINE_CACHE_DIR`) rather than the free tier,
because a wide sweep exposed two free-tier failure modes: free instances **spin
down after ~15 min idle** — killing a long-running screen and its in-memory
progress the moment polling pauses — and free has **no persistent disk**, so the
companyfacts cache evaporates on every restart and each run re-fetches thousands
of filings from SEC. Starter stays warm and keeps the cache, so a re-run after
an interruption resumes in seconds. Bump to `standard` if a very wide cold sweep
needs more memory headroom than Starter's 512 MB.

### Performance & scaling

A wide size-banded sweep is dominated by **per-name SEC fetches** — the size-cut
pulls one filing per filer to read its public float, then the screen pulls one
per surviving name. Three things keep that tractable:

- **Overlap** — fetches run on a bounded thread pool (`LANDMINE_MAX_WORKERS`), so
  round-trips pipeline instead of running one at a time (the old serial path was
  round-trip bound at ~1–2 names/s).
- **A shared rate limit** — a process-global limiter (`LANDMINE_SEC_RPS`, under
  SEC's ~10/s) paces the *aggregate* network rate across all workers, so
  parallelism never trips fair-access throttling. It paces only cache misses.
- **A persistent cache** — companyfacts JSON is cached to `LANDMINE_CACHE_DIR`
  with **atomic writes** (a unique temp file + rename), so concurrent fetchers
  can't corrupt an entry and a re-run on a persistent disk resumes in seconds.

The hard floor is SEC's ~10 req/s, so a *cold-cache* sweep of the whole
micro/small-cap band (~2,649 survivors out of ~10k filers sized) is still on the
order of minutes for its first run; subsequent runs are cache-fast. The biggest
remaining win is to size the universe from SEC's **`frames` API**
(`/api/xbrl/frames/dei/EntityPublicFloat/USD/CY####Q#I.json`), which returns the
public float for *every* filer in one request — collapsing the ~10k size-cut
fetches into a handful. It's left as a follow-up because frames key off the
reporting period rather than each fact's `filed` date, a small point-in-time
approximation to weigh against this repo's strict PIT guarantee before adopting.
