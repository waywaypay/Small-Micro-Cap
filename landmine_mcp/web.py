...
backend ``LANDMINE_API_KEY``), so it is protected and **fails closed**. There are
two modes; OAuth wins when configured:

* **OAuth** (Claude.ai connectors): set ``LANDMINE_MCP_PUBLIC_URL`` +
  ``LANDMINE_MCP_OAUTH_PASSWORD`` (see ``oauth.py``). The MCP SDK then mounts the
  OAuth endpoints and gates ``/mcp`` on a bearer token it issues; this module
  only adds ``/healthz`` and the ``/login`` consent page.
* **Legacy static bearer** (other hosts / testing), via this module's gate:
...
def _auth_mode() -> str:
    ...
    # posture different from what the gate actually enforces. OAuth (when the SDK
    # gates /mcp) takes precedence over the legacy bearer/disabled middleware.
...
def build_app():
    """Starlette ASGI app: /mcp + /healthz, gated by OAuth or the legacy bearer.

    OAuth mode: the SDK already mounts the metadata/authorize/token/register
    routes and wraps /mcp with its own bearer gate, so we only add the open
    /healthz probe and the /login consent page — no extra middleware.

    Legacy mode (no OAuth env): wrap everything in the bearer/disabled gate.
    """
    ...
    if oauth_provider is not None:
        inner.router.routes.extend(oauth_provider.routes())
        return inner
