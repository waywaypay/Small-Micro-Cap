"""Tests for the MCP server tools that wrap the landmine-api service.

These exercise the real (async) tool functions but route their HTTP calls
through the FastAPI app in-process via ``httpx.ASGITransport``, so they cover the
full wiring — header auth forwarding, as-of defaulting, payload shape, and error
surfacing — without a live network or a running server.
"""
import asyncio
...
def server(monkeypatch):
    ...
    # configure the MCP server, then route its AsyncClient at the app in-process
    ...
    monkeypatch.setattr(srv.httpx, "AsyncClient", make_client)
    return srv
...
def test_run_landmine_returns_scorecards(server):
    out = _run(server.run_landmine(["WKHS", "AAPL"], "2026-06-02"))
    assert out["count"] == 2
...
def test_unknown_ticker_surfaces_service_detail(server):
    with pytest.raises(RuntimeError) as exc:
        _run(server.run_landmine(["ZZZZNOPE"], "2026-06-02"))
...
def test_wrong_key_is_rejected(server, monkeypatch):
    ...
    with pytest.raises(RuntimeError) as exc:
        _run(server.run_landmine(["WKHS"], "2026-06-02"))
